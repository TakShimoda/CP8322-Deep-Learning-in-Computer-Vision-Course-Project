...
#for now, use this but we can replace sample_gumbel_like, gumbel_softmax_sample, and gum_softmax with F.gumbel_softmax()
...
class GateModule192(nn.Module):
    def __init__(self, act='relu', channels=24):
        super(GateModule192, self).__init__()

        self.inp_gs = GumbleSoftmax(hard=True)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.in_ch = in_ch = channels
        if act == 'relu':
            relu = nn.ReLU
        elif act == 'relu6':
            relu = nn.ReLU6
    ...
    def forward(self, x, temperature=1.):
        #input: (batch_size, W, H, 192)
        hatten = self.avg_pool(x) #tensor 2: (batch_size, 1, 1, 192)
        hatten_d = self.inp_gate(hatten) #tensor 3; this is output of SE block: (batch_size, 1, 1, 192)
        hatten_d = self.inp_gate_l(hatten_d) #tensor 4 step 1; is convolution with groups = 192: (batch_size, 1, 1, 192*2) 
        hatten_d = hatten_d.reshape(hatten_d.size(0), self.in_ch, 2, 1) #tensor 4 step 2; reshape to (batch, 192, 2, 1)
        hatten_d = self.inp_gs(hatten_d, temp=temperature, force_hard=True) #tensor 5; returns gumble sample (batch, 192, 2, 1)

        x = x * hatten_d[:, :, 1].unsqueeze(2) #multiply input by 
